...
const getTotal = async () => {
  ...
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.FOOD}`;
...
const getSelectOne = async (id) => {
  // const getTotal = async function () {
  try {
    const query = `SELECT COUNT(*) AS cnt FROM ${TABLE.FOOD} WHERE id=?`;
    const values = [id];
...
const getList = async (req) => {
  try {
    ...
    const query = `SELECT * FROM ${TABLE.FOOD} ${where} order by id desc limit 0, ${len}`;
...
const foodController = {
  ...
  create: async (req) => {
    ...
    try {
      const query = `INSERT INTO food (name, price) VALUES (?,?)`;
  ...
  update: async (req) => {
    ...
    const { name, price } = req.body;
    if (isEmpty(id) || isEmpty(name) || isEmpty(price)) {
      return resData(STATUS.E100.result, STATUS.E100.resultDesc, moment().format('LT'));
    }

    try {
      const query = `UPDATE ${TABLE.FOOD} SET name =?, price =? WHERE id= ?`;
  ...
  delete: async (req) => {
    ...
    try {
      ...
      const query = `DELETE FROM ${TABLE.FOOD} WHERE id = ?;`;
      const values = [id];
      const [rows] = await db.execute(query, values);
      if (rows.affectedRows == 1) {
        return resData(
